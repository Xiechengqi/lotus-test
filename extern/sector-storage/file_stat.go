...
func addCheckList(fReplica string, fCache string, sid abi.SectorID, ssize abi.SectorSize, checkList map[string]SectorFile) {
	checkList[fReplica] = SectorFile{
		Sid:  sid,
		Size: int64(ssize),
	}
	checkList[filepath.Join(fCache, "t_aux")] = SectorFile{
		Sid:  sid,
		Size: 0,
	}
	checkList[filepath.Join(fCache, "p_aux")] = SectorFile{
		Sid:  sid,
		Size: 0,
	}

	addCacheFilePathsForSectorSize(checkList, fCache, ssize, sid)
}
...
func checkBad(bad map[abi.SectorID]string, checkList map[string]SectorFile) error {
	...
	for _, v := range fList {
		...
		if v.Size == -1 { // not found
			log.Infof("file is not exist %s %d-%d", "/"+v.Name, p.Sid.Miner, p.Sid.Number)
			insert(bad, p.Sid, "file is not exist "+v.Name)
		} else if p.Size != 0 && p.Size != v.Size {
			log.Infof("file size is wrong", p.Size, v.Size, "/"+v.Name, p.Sid.Number, p.Sid.Miner)
